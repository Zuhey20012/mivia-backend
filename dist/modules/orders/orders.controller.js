"use strict";
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
      desc = { enumerable: true, get: function() { return m[k]; } };
    }
    Object.defineProperty(o, k2, desc);
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __setModuleDefault = (this && this.__setModuleDefault) || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable: true, value: v });
}) : function(o, v) {
    o["default"] = v;
});
var __importStar = (this && this.__importStar) || (function () {
    var ownKeys = function(o) {
        ownKeys = Object.getOwnPropertyNames || function (o) {
            var ar = [];
            for (var k in o) if (Object.prototype.hasOwnProperty.call(o, k)) ar[ar.length] = k;
            return ar;
        };
        return ownKeys(o);
    };
    return function (mod) {
        if (mod && mod.__esModule) return mod;
        var result = {};
        if (mod != null) for (var k = ownKeys(mod), i = 0; i < k.length; i++) if (k[i] !== "default") __createBinding(result, mod, k[i]);
        __setModuleDefault(result, mod);
        return result;
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
exports.createOrder = createOrder;
exports.listOrders = listOrders;
exports.getOrder = getOrder;
exports.createRental = createRental;
exports.listRentals = listRentals;
exports.getRental = getRental;
exports.createReturn = createReturn;
exports.getReturn = getReturn;
exports.updateReturnStatus = updateReturnStatus;
const orders_schema_1 = require("./orders.schema");
const ordersService = __importStar(require("./orders.service"));
// ─── ORDERS ──────────────────────────────────────────────────────────────────
async function createOrder(req, res) {
    const result = orders_schema_1.createOrderSchema.safeParse(req.body);
    if (!result.success)
        return res.status(400).json({ ok: false, errors: result.error.flatten() });
    try {
        const data = await ordersService.createOrder(req.user.id, result.data);
        res.status(201).json({ ok: true, ...data });
    }
    catch (e) {
        res.status(400).json({ ok: false, error: e.message });
    }
}
async function listOrders(req, res) {
    const orders = await ordersService.listOrders(req.user.id);
    res.json({ ok: true, orders });
}
async function getOrder(req, res) {
    try {
        const order = await ordersService.getOrder(Number(req.params.id), req.user.id);
        res.json({ ok: true, order });
    }
    catch (e) {
        res.status(e.message === "Forbidden" ? 403 : 404).json({ ok: false, error: e.message });
    }
}
// ─── RENTALS ─────────────────────────────────────────────────────────────────
async function createRental(req, res) {
    const result = orders_schema_1.createRentalSchema.safeParse(req.body);
    if (!result.success)
        return res.status(400).json({ ok: false, errors: result.error.flatten() });
    try {
        const data = await ordersService.createRental(req.user.id, result.data);
        res.status(201).json({ ok: true, ...data });
    }
    catch (e) {
        res.status(400).json({ ok: false, error: e.message });
    }
}
async function listRentals(req, res) {
    const rentals = await ordersService.listRentals(req.user.id);
    res.json({ ok: true, rentals });
}
async function getRental(req, res) {
    try {
        const rental = await ordersService.getRental(Number(req.params.id), req.user.id);
        res.json({ ok: true, rental });
    }
    catch (e) {
        res.status(e.message === "Forbidden" ? 403 : 404).json({ ok: false, error: e.message });
    }
}
// ─── RETURNS ─────────────────────────────────────────────────────────────────
async function createReturn(req, res) {
    const result = orders_schema_1.createReturnSchema.safeParse(req.body);
    if (!result.success)
        return res.status(400).json({ ok: false, errors: result.error.flatten() });
    try {
        const ret = await ordersService.createReturn(req.user.id, result.data);
        res.status(201).json({ ok: true, return: ret });
    }
    catch (e) {
        res.status(400).json({ ok: false, error: e.message });
    }
}
async function getReturn(req, res) {
    try {
        const ret = await ordersService.getReturn(Number(req.params.id), req.user.id);
        res.json({ ok: true, return: ret });
    }
    catch (e) {
        res.status(e.message === "Forbidden" ? 403 : 404).json({ ok: false, error: e.message });
    }
}
async function updateReturnStatus(req, res) {
    const result = orders_schema_1.updateReturnStatusSchema.safeParse(req.body);
    if (!result.success)
        return res.status(400).json({ ok: false, errors: result.error.flatten() });
    try {
        const ret = await ordersService.updateReturnStatus(Number(req.params.id), result.data);
        res.json({ ok: true, return: ret });
    }
    catch (e) {
        res.status(400).json({ ok: false, error: e.message });
    }
}
